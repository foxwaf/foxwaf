...
<p align="center"><img src="./screenshot/login.png" width="800" alt="登录页面"/></p>

### 数据概览

实时监控总请求数、拦截数、缓存命中率、规则总数、站点数，一目了然。

<p align="center"><img src="./screenshot/dashboard.png" width="800" alt="数据概览"/></p>

支持暗黑主题，长时间运维不伤眼。

<p align="center"><img src="./screenshot/dashboard_dark.png" width="800" alt="暗黑主题"/></p>

### 站点管理

添加、编辑、删除站点，配置上游地址、HTTPS、负载均衡、HTTP/2 等，所有变更热加载生效。

<p align="center"><img src="./screenshot/sites.png" width="800" alt="站点管理"/></p>

站点编辑支持完整的反向代理参数配置：

<p align="center"><img src="./screenshot/site_edit.png" width="800" alt="站点编辑"/></p>

在线管理 SSL/TLS 证书，上传后自动热加载，HTTPS 连接无感切换：

<p align="center"><img src="./screenshot/site_cert.png" width="800" alt="证书管理"/></p>

### 攻击日志

详细记录每一次攻击拦截，支持按攻击类型、IP、域名、规则等多维度筛选和搜索。

<p align="center"><img src="./screenshot/attack_logs.png" width="800" alt="攻击日志"/></p>

<p align="center"><img src="./screenshot/attack_logs2.png" width="800" alt="攻击日志筛选"/></p>

点击任意日志可查看完整攻击详情，包括请求头、匹配规则、攻击载荷、GeoIP 等：

<p align="center"><img src="./screenshot/attack_detail.png" width="800" alt="攻击详情"/></p>

### 拦截页面

命中 WAF 规则后，攻击者会看到拦截提示页面：

<p align="center"><img src="./screenshot/waf_block_page.png" width="800" alt="WAF 拦截页面"/></p>

### ACL 访问控制

支持全局和站点级 IP 黑白名单，规则变更即时生效，无需重启。

<p align="center"><img src="./screenshot/acl_list.png" width="800" alt="ACL 列表"/></p>

<p align="center"><img src="./screenshot/acl_add.png" width="800" alt="添加 ACL"/></p>

<p align="center"><img src="./screenshot/acl_edit.png" width="800" alt="编辑 ACL"/></p>

### CC 防护

基于路径 + 域名的频率限制，支持 block（直接拦截）和 challenge（JS 挑战验证）两种动作。

<p align="center"><img src="./screenshot/cc_list.png" width="800" alt="CC 规则列表"/></p>

<p align="center"><img src="./screenshot/cc_add.png" width="800" alt="添加 CC 规则"/></p>

<p align="center"><img src="./screenshot/cc_edit.png" width="800" alt="编辑 CC 规则"/></p>

触发 CC 防护时，用户需通过 JS Challenge 或滑块验证：

<p align="center"><img src="./screenshot/cc_challenge1.png" width="800" alt="CC JS Challenge"/></p>

<p align="center"><img src="./screenshot/cc_challenge2.png" width="800" alt="CC 滑块验证"/></p>

<p align="center"><img src="./screenshot/cc_block_page.png" width="800" alt="CC 拦截页面"/></p>

### 自定义规则

可视化规则编辑器，支持多条件组合（URI / 参数 / Header / Body），contains / regex / equals 等匹配方式。规则热加载，保存即生效。

<p align="center"><img src="./screenshot/custom_rules.png" width="800" alt="自定义规则列表"/></p>

<p align="center"><img src="./screenshot/custom_rule_add.png" width="800" alt="添加自定义规则"/></p>

### 缓存管理

内置静态资源缓存，按域名维度管理，支持一键清除。

<p align="center"><img src="./screenshot/cache.png" width="800" alt="缓存管理"/></p>

### 已禁用规则

误报规则可一键禁用，不影响其他防护，随时可恢复启用。

<p align="center"><img src="./screenshot/disabled_rules.png" width="800" alt="已禁用规则"/></p>

### 插件管理

插件系统支持热加载 / 热卸载，扩展 User-Agent 校验、响应内容审计等能力，不中断任何请求。

<p align="center"><img src="./screenshot/plugins.png" width="800" alt="插件管理"/></p>

### 系统设置

全局安全开关：反爬虫、UA 检查、反 DevTools、规则匹配灵敏度、解码深度等，修改即时生效。

<p align="center"><img src="./screenshot/settings.png" width="800" alt="系统设置"/></p>

### 面板设置

修改管理员密码、安全入口、面板端口，在线完成运维操作。

<p align="center"><img src="./screenshot/panel_settings.png" width="800" alt="面板设置"/></p>

### 反馈系统

内置反馈通道，用户可在拦截页面提交误报反馈，管理员在面板中审核处理。

<p align="center"><img src="./screenshot/feedback_submit.png" width="800" alt="提交反馈"/></p>

<p align="center"><img src="./screenshot/feedback_list.png" width="800" alt="反馈列表"/></p>

### 跳转事件

记录并展示所有 301/302 跳转事件，便于排查配置问题。

<p align="center"><img src="./screenshot/redirect_event.png" width="800" alt="跳转事件"/></p>

### 命令行工具

`foxwaf` CLI 提供完整的服务管理能力：

<p align="center"><img src="./screenshot/foxwaf_status.png" width="800" alt="foxwaf status"/></p>

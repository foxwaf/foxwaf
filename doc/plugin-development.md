...
```bash
# 上传 zip
curl -H "X-Dev-Plain: 1" -b cookies.txt -c cookies.txt \
     -F "file=@myplugin-1.0.0.zip" \
     http://<panel-host>:8088/api/plugins/upload

# URL 下载
curl -H "X-Dev-Plain: 1" -b cookies.txt -c cookies.txt \
     -H "Content-Type: application/json" \
     -d '{"url":"https://example.com/myplugin-1.0.0.zip"}' \
     http://<panel-host>:8088/api/plugins/import-url
```
